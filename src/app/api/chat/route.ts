...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (session.state) {
      ...
      case 'diagnosing': {
        // 检测是否有预约/看诊意向
        const bookingKeywords = ['预约', '挂号', '看诊', '面诊', '去医院', '想去看', '推荐医院', '哪家医院', '哪个医院', '去哪看', '怎么预约'];
        const wantsBooking = bookingKeywords.some(k => message.includes(k));

        // 检测是否选择了医院（数字或医院名）
        const hospitalNumMatch = message.match(/^[1-3]$/);

        if (wantsBooking || (session.messages.length > 4 && (message.includes('想') || message.includes('要')))) {
          // 提取症状关键词
          ...
          if (hospitals.length > 0) {
            reply = '根据您的情况，我推荐以下医院供您参考：\n\n';
            hospitals.forEach((h, i) => {
              reply += `${i + 1}. ${h.name}\n   ⭐ ${h.rating}分 · ${h.address}\n   🕐 ${h.hours}\n   📞 ${h.phone}\n   💡 ${h.description}\n\n`;
            });
            reply += '请问您想选择哪家医院？回复序号（1/2/3）即可，或者告诉我您的偏好。';
            newState = 'recommend_hospital';
          } else {
            reply = '抱歉暂时没有匹配的医院数据，建议您就近选择正规口腔医院就诊。需要我帮您预约吗？';
          }
          break;
        }
